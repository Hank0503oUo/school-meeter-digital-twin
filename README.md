...
這個專案需要特別注意的地方，是它包含真實或近真實能源資料工作流。
...
這也反過來說明這個專案做過真正的資料治理與交付邊界設計，而不是單純把研究檔案直接放上 GitHub。
...
這個 repository 目前以 architecture-focused README 為主。完整版本原本包含 dashboard、MCP、推論引擎、知識工作台與本地模型整合，但這些內容不適合直接完整公開，因為涉及校園資料、衍生索引與本地模型資產。
